...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_gre_label
global VCtl
global VVar
CV1=0;
CV10=0;
...
[rfAmp1,rfPhase1,rfFreq1,rfCoil1,rfTime1]=rfUser(p);
...
p.Duplicates=1;
p.GzFile='gz.mat';
p.Notes='PyPulseq Gz waveform';
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
if strcmp(p.Switch,'on')
[GzAmp1,GzTime1]=GzUser(p);
GzAmp=[GzAmp GzAmp1];
...
p.Duplicates=1;
p.GyFile='gy.mat';
p.Notes='PyPulseq Gy waveform';
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
if strcmp(p.Switch,'on')
[GyAmp1,GyTime1]=GyUser(p);
GyAmp=[GyAmp GyAmp1];
...
p.Duplicates=1;
p.GxFile='gx.mat';
p.Notes='PyPulseq Gx waveform';
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
if strcmp(p.Switch,'on')
[GxAmp1,GxTime1]=GxUser(p);
GxAmp=[GxAmp GxAmp1];
...
p.DupSpacing=0;
p.Duplicates=1;
p.Notes='PyPulseq ADC waveform';
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
if strcmp(p.Switch,'on')
[ADC1,ADCTime1]=ADCUser(p);
ADC=[ADC ADC1];
